        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-xs text-slate-400">District: {currentUser.district || 'Head Office'}</p>
             </div>
          </div>
                         ...
                         <tr key={agent.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-4 text-sm font-mono text-slate-600">
                               <div className="flex items-center gap-2">
                                 <span className="font-bold text-indigo-700">{agent.username}</span>
                               </div>
                               <div className="flex items-center gap-2 mt-1">
                                 <span className="text-xs text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded border">Pass: {agent.password}</span>
                                 <button 
                                   onClick={() => copyToClipboard(`ID: ${agent.username} Pass: ${agent.password}`)}
...
const App: React.FC = () => {
  ...
  // Flow Logic:
  // 1. User Logged In -> Dashboard
  // 2. showLogin is true -> Login Component
  ...
  return (
    <>
      <PublicLanding onNavigateLogin={() => setShowLogin(true)} />
      <AIHelp />
    </>
  );
};
